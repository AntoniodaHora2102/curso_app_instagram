...
class GalleryFragment : BaseFragment<FragmentGalleryBinding, Post.Presenter>(
    ...
    override fun setupViews() {
        binding?.galleryRv?.layoutManager = GridLayoutManager(requireContext(), 3) // quantidade de grids que iremos trabalhar
    ...
    override fun displayFullPictures(posts: List<Uri>) {
        ...
        binding?.galleryImgSelected?.setImageURI(posts.first())//ira acessar a primeira
        binding?.galleryNested?.smoothScrollTo(0,0) // ira rolar a imagem selecionada para cima
        presenter.selectUri(posts.first()) //indice do post
    }
    ...
    //irá notificar o fragment quando o BUTTON for clicado
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
      when(item.itemId) {
          R.id.action_send -> {
          setFragmentResult("takePhotoKey",
